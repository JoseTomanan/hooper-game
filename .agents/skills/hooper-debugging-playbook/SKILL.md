...
| Grep/Glob returns duplicate hits across the repo | `.Codex/worktrees/` contains 10+ stale full worktree copies of the repo (leftover from past parallel-agent sessions) that are NOT excluded by default from a repo-root search. | Scope the `path`/`glob` argument away from `.Codex/worktrees/` (e.g. search under `scripts/`, `tests/`, `scenes/` explicitly rather than the repo root) instead of trying to dedupe results after the fact. |
...
`.Codex/worktrees/` (11 entries at verification time — table says "10+", re-run
`ls .Codex/worktrees | wc -l` if this drifts). Cross-referenced against sibling
...
- `ls "C:/Users/The King/Documents/GitHub/hooper-game/.Codex/worktrees" | wc -l`
