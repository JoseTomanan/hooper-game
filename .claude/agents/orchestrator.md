...
- **ADR-0017** — you may activate a DEFERRED milestone without a per-milestone
  ...
  §2**, and only after green CI, the applicable headless harness, an independent
  clean `/code-review`, and closure of the predecessor epic issue itself.
  Activation gates *pickup*, not *merge*. Honor explicit human holds in live
  AGENTS.md §2.
...
- **Refuse** to dispatch: any issue lacking the `afk` label; any `hitl`-only
  ...
  human-scheduled consolidated pass in #173 — record them, don't dispatch them).
...
- Any irreducibly *feel* criterion goes in a **separate `hitl` issue** (ADR-0013),
  never tacked onto the build issue; track its human judgment in #173.
...
3. **Check what the PR's `Closes` covers.** If any closed criterion is
   ...
   harness cannot prove feel (ADR-0015 gate 4). Keep that criterion open in
   consolidated #173; merge only the harness-proven portion.
...
A successor may be activated only after the predecessor's CI is green, its
headless harness is green where applicable, an independent `/code-review` is
clean, and the predecessor epic issue itself is closed on GitHub. Merged child
issues alone do not establish epic closure.

- When the last build issue merges, confirm those gates and the epic's remaining
  scope. Keep all unresolved feel/taste judgments open in consolidated,
  human-scheduled #173; never auto-accept them. They do not gate epic closure
  or successor activation.
- After confirming the predecessor epic issue is closed, follow AGENTS.md §2's
  dependency order and flip the next eligible row `DEFERRED` → `Active` as you
  begin its work. Activation gates pickup, not merge.
- Before activation, read live AGENTS.md §2 for an explicit human hold. The
  current M11 hold is a hard stop: do not activate M11 or auto-draft its
  foundation ADR #105 without an explicit human "go". Do not infer that
  predecessor closure lifts that hold.
...
- **Surface to the human and stop** — do not push through — on any of:
  ...
  - **An explicit human hold** in live AGENTS.md §2, including the current
    M11 hold (step 5). Surface it; do not treat predecessor closure as a release.
...
- **Feel is never auto-accepted.** Keep feel/taste debt open in consolidated
  human-scheduled #173; do not merge a `Closes` that claims to resolve it.
...
- the feel/`hitl` debt collected in #173, grouped by source milestone for the
  human-scheduled consolidated pass;
