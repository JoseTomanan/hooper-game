extends SceneTree
# Asset build tool (#304) — drafts the jab-step clip family into
# assets/locomotion.res by SLICING assets/jabstep_authored.fbx.
#
# Run:  godot --headless --path . -s tools/rebuild_jabstep_clips.gd
...
# one lands).
#
# Produces THREE LOOP_NONE one-shots — jab step is UNHANDED, and per
# author_jabstep.py's own module docstring it commits to ONE fixed jab-leg
# polarity, so — like the layup (#313) and contest (#314) families and unlike
# the dribble-move family — this tool slices THREE clips, not six:
#   jabstepstartup    3 ticks / 0.05000 s
#   jabstepactive     2 ticks / 0.03333 s
#   jabsteprecovery   4 ticks / 0.06667 s
#
# ── The smallest committed move in the game ──────────────────────────────────
# 9 ticks / 0.150 s total. A 2-tick Active window is one to two rendered
# frames at 60 Hz, so per README's "the <=3-tick segments are single poses"
# rule, author_jabstep.py authors THREE HELD POSES rather than three little
# movies — the read is pose CONTRAST between phases. #304's own segment-length
# gate matters more here than anywhere else in the batch: an off-by-one on a
# 2-tick clip is a 50% length error, not a rounding nicety.
#
# ── Why this is a SLICE, not a compose ───────────────────────────────────────
# tools/author_jabstep.py (headless Blender, #315's blender_anim_lib machinery)
...
# rebuild_jumpshot_clips.gd's `_slice()` primitive, copied via
# rebuild_contest_clips.gd's verbatim adaptation — and then PROVE
...
# purpose (rebuild_contest_clips.gd's own header states the reason): the FBX
# round-trip, the importer's fps/trimming/immutable-track settings, and
# `_slice`'s resampling are exactly the machinery that has silently corrupted
# clips in this repo before (#281, #295, #297).
#
...
# so a silent zero-match can never hide behind a green run (verbatim
# rebuild_contest_clips.gd/rebuild_steal_clips.gd approach).
#
...
# skeleton's rest transform. author_jabstep.py's own
...
# imported from jabstep_authored.fbx reads
...
# Hardcoded, not derived: author_jabstep.py's frame layout is DETERMINISTIC BY
# CONSTRUCTION — it keys its timeline at exact times computed from JabStep's
# own frame data (3/2/4 ticks @ 60 Hz) and the import sets `trimming=false`,
...
# constant and changes no gameplay behaviour. JabStepLegalityResolver.IsLegal
# and every BallState gate PlayerController.BeginCommittedMove enforces are
# untouched — the tick counts below are DUPLICATED from JabStep's frame data
# for slicing, never read back into it.

const LIB_PATH := "res://assets/locomotion.res"
const SRC_FBX := "res://assets/jabstep_authored.fbx"
# Matches author_jabstep.py's ACTION_NAME -- export_fbx() renames both the
# Blender action AND the scene to this so Godot's importer names the
# resulting AnimationPlayer take after it.
const SRC_CLIP := "jabstep"
...
# JabStep's frame data (scripts/Input/JabStep.cs DefaultFrameData). Duplicated
# here because GDScript cannot read the C# constant -- so the duplication is
# made SAFE rather than avoided: JabStepAnimTest's `jabstep-segment-lengths`
# scenario asserts each clip's length equals JabStep.DefaultFrameData's own
...
# Source-time windows, matching author_jabstep.py's frame table exactly (frame
# numbers there ARE physics ticks at 60 Hz: 0/3/5/9).
...
# author_jabstep.py), so the imported clip's length must be ~9/60 s. A
...
const NAMES := ["jabstepstartup", "jabstepactive", "jabsteprecovery"]

# G3 legibility floor (#296's actual complaint -- Startup and Recovery must
# read as visibly different poses). Matches author_jabstep.py's own
...
# slice untouched. Measured on the authored source: 37.407 deg.
const STARTUP_VS_RECOVERY_MIN_DEG := 15.0

# G4: torso pitches FORWARD by Active, the load-bearing contrast with retreat
# dribble (#305) per the issue's motion spec. Measured against the HIPS-frame
# spine->head vector's projection along `forward` — the same axis convention
# author_jabstep.py's own `_torso_pitch_sign_is_forward` oracle uses, re-proven
# here on the SLICED Godot resource rather than trusted from the Blender side.
const SPINE_BONE := "mixamorig_Spine"
const HEAD_BONE := "mixamorig_Head"
# Minimum forward growth (Active vs Startup) of the spine->head vector's
# projection along `forward`, in metres. Small on purpose -- a 10 deg pitch of
# a ~0.6 m spine moves this by only a few centimetres; the gate exists to
# catch a SIGN error or a dead clip, not to demand a specific pitch magnitude.
const TORSO_FORWARD_GROWTH_MIN_M := 0.01
...
# object, so a track imported from jabstep_authored.fbx reads
...
func _initialize() -> void:
	...
	if lib == null or not (lib is AnimationLibrary):
		push_error("[rebuild-jabstep] failed to load AnimationLibrary at %s" % LIB_PATH)
		quit(1)
		return

	_skel = _find(load("res://assets/Y Bot.fbx").instantiate(), "Skeleton3D")
	if _skel == null:
		push_error("[rebuild-jabstep] could not find a Skeleton3D in Y Bot.fbx.")
		quit(1)
		return
	...
	if packed == null:
		push_error("[rebuild-jabstep] failed to load %s" % SRC_FBX)
		quit(1)
		return
	var ap: AnimationPlayer = _find(packed.instantiate(), "AnimationPlayer")
	if ap == null:
		push_error("[rebuild-jabstep] %s has no AnimationPlayer." % SRC_FBX)
		quit(1)
		return
	if not ap.has_animation(SRC_CLIP):
		push_error("[rebuild-jabstep] %s has no AnimationPlayer clip '%s' (has: %s)"
			% [SRC_FBX, SRC_CLIP, str(ap.get_animation_list())])
		quit(1)
		return
	var src: Animation = ap.get_animation(SRC_CLIP)
	print("[rebuild-jabstep] source '%s': len=%.5f tracks=%d"
	...
	if absf(src.length - EXPECTED_SRC_LENGTH_S) > SRC_LENGTH_TOL_S:
		push_error("[rebuild-jabstep] source length %.5f s is not within %.3f s of the expected "
	...
	for i in src.get_track_count():
		...
		if ty == Animation.TYPE_ROTATION_3D:
			src_rot += 1
	print("[rebuild-jabstep] source has %d tracks; %d expected per slice after dropping "
	...
	for name in built:
		...
		print("[rebuild-jabstep] G1 '%s': length=%.5f (expect %.5f) loop_mode=%d %s"
			% [name, anim.length, expected_len, anim.loop_mode, "OK" if (len_ok and loop_ok) else "FAIL"])
		if not (len_ok and loop_ok):
			g1_ok = false
	if not g1_ok:
		push_error("[rebuild-jabstep] G1 FAILED -- one or more clips has the wrong length or loop_mode.")
		quit(1)
		return
	...
	for name in built:
		if not _assert_complete(built[name], name, src_rot, src_total, form_counts):
			g2_ok = false
	print("[rebuild-jabstep] G2 bone-name resolution across all three clips: as-given=%d alt=%d unresolved=%d"
		% [form_counts["as-given"], form_counts["alt"], form_counts["unresolved"]])
	if not g2_ok:
		quit(1)
		return
	if form_counts["unresolved"] > 0:
		push_error("[rebuild-jabstep] G2 FAILED -- %d bone tracks did not resolve on Y Bot in EITHER "
			% form_counts["unresolved"] + "name form.")
		quit(1)
		return
	var resolved_total: int = form_counts["as-given"] + form_counts["alt"]
	if resolved_total <= 0:
		push_error("[rebuild-jabstep] G2 FAILED -- resolved count is %d; a name-matching gate that "
	...
	print("[rebuild-jabstep] G3 startup-end vs recovery-end max bone delta = %.1f deg (want >= %.1f)"
		% [g3_delta, STARTUP_VS_RECOVERY_MIN_DEG])
	if g3_delta < STARTUP_VS_RECOVERY_MIN_DEG:
		push_error("[rebuild-jabstep] G3 FAILED: only %.1f deg (< %.1f) -- Startup's end pose and "
			% [g3_delta, STARTUP_VS_RECOVERY_MIN_DEG] + "Recovery's end pose do not read as distinct (#296).")
		quit(1)
		return

	# ── G4: torso pitches FORWARD by Active (the retreat-dribble contrast) ──
	# Startup's OWN end pose vs Active's END pose (the stab instant), not vs a
	# rest reference -- consistent with G3's "compare named phase instants,
	# not whole-clip endpoints" discipline.
	...
	print("[rebuild-jabstep] G4 torso-forward growth Startup->Active = %.4f m (want >= %.4f); startup=%.4f active=%.4f"
		% [growth, TORSO_FORWARD_GROWTH_MIN_M, startup_lean, active_lean])
	if growth < TORSO_FORWARD_GROWTH_MIN_M:
		push_error("[rebuild-jabstep] G4 FAILED: torso-forward growth is only %.4f m (< %.4f) -- the jab is "
			% [growth, TORSO_FORWARD_GROWTH_MIN_M] + "supposed to pitch FORWARD into the stab (#304's "
			+ "defining contrast with retreat dribble, #305). Check TORSO_PITCH_SIGN in author_jabstep.py.")
		quit(1)
		return
	...
	if err != OK:
		push_error("[rebuild-jabstep] ResourceSaver.save failed with error %d" % err)
		quit(1)
		return

	print("[rebuild-jabstep] saved %s; clips now: %s" % [LIB_PATH, str(lib.get_animation_list())])
	quit(0)
...
func _derive_body_axes() -> bool:
	...
	if foot < 0 or toe < 0:
		push_error("[rebuild-jabstep] Y Bot has no LeftFoot/LeftToeBase to derive facing from.")
		return false
	var forward: Vector3 = _skel.get_bone_global_rest(toe).origin - _skel.get_bone_global_rest(foot).origin
	forward.y = 0.0
	if forward.length() < 0.001:
		push_error("[rebuild-jabstep] Y Bot's foot->toe vector is vertical; cannot derive facing.")
		return false
	...
	if lh < 0 or rh < 0:
		push_error("[rebuild-jabstep] Y Bot has no LeftHand/RightHand to verify the lateral axis.")
		return false
	...
	print("[rebuild-jabstep] axes: forward=%s right=%s | rest hand lateral L=%+.4f R=%+.4f"
		% [_forward, _right, lh_lat, rh_lat])
	if not (rh_lat > 0.0 and lh_lat < 0.0):
		push_error("[rebuild-jabstep] the derived right axis puts the RIGHT hand at %+.4f and the LEFT "
...
# metres. Used by G4 -- the same quantity author_jabstep.py's own
# `_torso_pitch_sign_is_forward` oracle measures Blender-side, re-measured here
# on the SLICED Godot resource by manual FK.
...
func _assert_complete(anim: Animation, name: StringName, expected_rot: int, expected_total: int, form_counts: Dictionary) -> bool:
	...
	for i in anim.get_track_count():
		...
		if res[0] < 0:
			unresolved.append(b)
	print("[rebuild-jabstep]   '%s': len=%.4f tracks=%d rot=%d loop=%d unresolved=%s"
	...
	if rot != expected_rot or anim.get_track_count() != expected_total:
		push_error("[rebuild-jabstep] '%s' has %d/%d tracks, source has %d/%d -- a slice must inherit "
	...
	if unresolved.size() > 0:
		push_error("[rebuild-jabstep] '%s' has tracks that do not resolve on Y Bot in EITHER name form: %s"
			% [name, str(unresolved)])
		return false
	if bad_shape.size() > 0:
		push_error("[rebuild-jabstep] '%s' has %d track(s) whose NODE PATH cannot bind on "
			% [name, bad_shape.size()]
...
# kinematics (verbatim rebuild_contest_clips.gd / rebuild_steal_clips.gd
