...
public partial class DedicatedGameJourneyTest : Node
{
	...
	private double _discoveryObservationStartedAt = -1.0;
	...
	private void TickServer()
	{
		...
		{
			if (_discoveryBroadcastCountAtListenerReady < 0 && Exists("client-a-discovery-listening"))
				_discoveryBroadcastCountAtListenerReady = _broadcaster.SuccessfulBroadcastCountForHarness;
			if (_discoveryBroadcastCountAtListenerReady >= 0
				&& _broadcaster.SuccessfulBroadcastCountForHarness > _discoveryBroadcastCountAtListenerReady
				&& !Exists("server-beacon-after-listener-ready"))
			{
				Write("server-beacon-after-listener-ready", $"count={_broadcaster.SuccessfulBroadcastCountForHarness}");
			}
			if (Exists("client-a-pass") && Exists("server-beacon-after-listener-ready"))
				Pass($"discovery-disabled client stayed empty while server emitted {_broadcaster.SuccessfulBroadcastCountForHarness} beacon(s)");
			return;
		}
	...
	private void TickDiscoveryNegative()
	{
		...
		if (_browser.FindRowForHarness("127.0.0.1", _port) >= 0
			|| _browser.RowCountForHarness != 0 || _browser.ServerListUi.ItemCount != 0)
		{
			Fail($"discovery-disabled mutation unexpectedly produced a browser row; backing={_browser.RowCountForHarness}, ui={_browser.ServerListUi.ItemCount}.");
			return;
		}
		if (!_discoveryListeningWritten)
		{
			_discoveryListeningWritten = true;
			Write("client-a-discovery-listening", $"port={_discoveryPort + 1}");
		}
		if (!Exists("server-beacon-after-listener-ready")) return;
		if (_discoveryObservationStartedAt < 0.0)
			_discoveryObservationStartedAt = _elapsed;
		if (_elapsed - _discoveryObservationStartedAt >= DiscoveryNegativeSeconds)
			Pass($"listener bound {_discoveryPort + 1}; target beacon on {_discoveryPort} then produced no browser row for {DiscoveryNegativeSeconds:F1}s");
	}
	...
	private void TryActivateExpectedBrowserRow()
	{
		...
		GD.Print($"[dedicated-game] {_role} activating exact browser row {row}: 127.0.0.1:{_port}.");
