...
public class HeadingMathTests
{
    ...
    private const float MaxTurnDeg      = 540f;
    ...
    {
        // A 5° difference with MaxTurnDeg=540 and delta=1/60: max step per tick
        // is 540/60 = 9° — well above 5°, so one tick must land exactly on target.
    ...
    {
        // A 170° difference: one tick at 540 °/s × 0.35 slow-factor (near-180°)
        // ≈ 3.15 °/tick — nowhere near 170°. The result must be strictly between
    ...
    {
        // A 180° (π) back-turn at BackTurnSlow=0.35: max step ≈ 540×0.35/60 ≈ 3.15 °.
    ...
    public void RotateToward_RepeatedCalls_EventuallyReachTarget()
    {
        ...
        // 120 ticks = 2 s at 60 Hz — a 180° turn at minimum rate takes ~1 s.
        float currentYaw = 0f;
